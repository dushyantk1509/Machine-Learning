...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        ############################################################################
        #                       FORWARD PROPAGATION                                #
        affine_transform = features.dot(W_proj) + b_proj            # STEP 1 (N,H)
    ...
    def sample(self, features, max_length=30):
        ...
        # a loop.                                                                 #
        ###########################################################################
        next_caption = np.zeros((N,1),dtype=np.int32) + self._start
        ...
        for i in range(max_length):
            ...
            temporal_affine_transform, _ = temporal_affine_forward(word_transform, W_vocab, b_vocab)    #  (N,1,V)
            #print(temporal_affine_transform.shape)
            next_caption = np.argmax(temporal_affine_transform,axis=2).reshape(-1)
            captions[:,i] = next_caption
            next_caption = next_caption.reshape(next_caption.shape[0],1)
            word_transform = word_transform.reshape(N,H)
        pass
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
